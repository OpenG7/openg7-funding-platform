...
| --- | --- | --- |
| Entreprise choisit une commandite, paie et revient avec token | Oui | Fonds, Checkout, API Stripe sans token brut en metadata |
| Entreprise rouvre le suivi par token et soumet ses details | Oui | Suivi commandite, API token, retrait du token de l'URL |
...
| Admin approuve, remet en attente ou refuse la commandite | Oui | Admin, revue DB |
